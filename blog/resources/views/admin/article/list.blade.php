    ...
    <script>
        $(document).ready(function () {
            var table = $('#dataTables-example').DataTable({
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row, meta) {
                            if (row.up == 1) {
                                return '<img src="{{asset("images/pin_3.gif")}}" />';
                            } else {
                                return '';
                            }
                        }
                    ...
                    },
                    {
                        ...
                        orderable: false,
                        render: function (data, type, row, meta) {
                            return "<a class='glyphicon glyphicon-edit' href='/admin/article/" + row.id + "/edit' title='编辑'></a>" +
                                    "&nbsp;&nbsp;&nbsp;&nbsp;" +
                ...
                createdRow: function (row, data, index) {
                    $('td:eq(2),td:eq(3),td:eq(4),td:eq(5),td:eq(7)', row).css('text-align', "center");
                },
                initComplete: function () {
                    //表格加载完毕，手动添加按钮到表格上
                    $("#toolbar").parent().find('label').css("float", "left").css("margin-right", "10px");
                    $("#toolbar").append("<a href='/admin/article/create' class='btn btn-primary btn-sm'>添加文章</a>");
                }
            });
            ...
            $('#dataTables-example tbody').on('click', '.delRow', function () {
                ...
                swal({
                            title: "Are you sure?",
                            text: "您确认要删除『 " + row.data().title + " 』这篇文章么",
                            type: "warning",
